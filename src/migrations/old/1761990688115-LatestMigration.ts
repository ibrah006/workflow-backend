...
export class LatestMigration1761990688115 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            ...
            BEGIN
              IF NOT EXISTS (
                SELECT 1 FROM pg_type WHERE typname = 'material_log_type_enum'
              ) THEN
                CREATE TYPE "material_log_type_enum" AS ENUM ('IN', 'OUT', 'ESTIMATE');
              END IF;
            END
            $$;
          `);
        ...
        await queryRunner.query(`
            ...
            BEGIN
              IF NOT EXISTS (
                SELECT 1 FROM pg_type WHERE typname = 'materials_measuretype_enum'
              ) THEN
                CREATE TYPE "materials_measuretype_enum" AS ENUM (
                  'running_meter',
                  'item_quantity',
                  'liters',
                  'kilograms',
        ...
        await queryRunner.query(`
            ...
            BEGIN
              IF NOT EXISTS (
                SELECT 1 FROM pg_type WHERE typname = 'stock_transactions_type_enum'
              ) THEN
                CREATE TYPE "stock_transactions_type_enum" AS ENUM (
                  'stock_in',
                  'stock_out',
                  'adjustment'
                );
            ...
            END$$;
          `);
        await queryRunner.query(`CREATE TABLE IF NOT EXISTS "stock_transactions" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "materialId" uuid NOT NULL, "type" "public"."stock_transactions_type_enum" NOT NULL, "quantity" numeric(10,2) NOT NULL, "balanceAfter" numeric(10,2) NOT NULL, "barcode" character varying, "notes" text, "projectId" character varying, "createdById" uuid NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_1aa2430f5ac950c26da6e1ff222" PRIMARY KEY ("id"))`);
        await queryRunner.query(`
            DO $$
            BEGIN
              IF NOT EXISTS (
                SELECT 1 FROM pg_type WHERE typname = 'invitations_status_enum'
              ) THEN
